...
interface CurrentSchoolinfoScreenProps { navigation: any, route: any }
...
const CurrentSchoolinfo = (props: CurrentSchoolinfoScreenProps, StatusBarProps: StatusBarProps) => {
  ...
  const onPressSubmit = async () => {
    ...
    if (
      stateError ||
      cityError || schoolError || schoolnameError || BoardError || zatchupidError || addressError
    ) {
      //this._scrollView.scrollTo(0);
      Toast.show(stateError || cityError || schoolError || schoolnameError || BoardError || zatchupidError || addressError, Toast.SHORT);
  ...
  const submit = async (data) => {
    ...
    {
      schooldatkey != '0' ? props.navigation.navigate('Onboarded', { data: props.route.params.data, 'school_id': schoolKey[0].value, 'nameofschool': schoolKey[0].label, 'school_zatchup_id': ID, 'state': stateKey[0].label, 'city': cityKey[0].label, 'address': address, 'board': Board }) : props.navigation.navigate('AddCourseDetailsOthers', { 'nameofschool': schoolname, 'board': Board, 'school_id': data.school_id })
    }
  ...
  return (
    ...
    <View style={styles.maincontainer}>
      ...
      {props.route.params.data === true ? <CustomHeader Title={'Add Current School'} /> : <CustomHeader Title={'Add School'} />}

      <View style={styles.fillTextContainer}><Text style={styles.fillText}>Please Fill in the details below:</Text></View>

      <View style={{ marginBottom: '5%', marginLeft: 25, marginRight: 25 }}>

        <TextField placeholder={'Enter ZatchUp ID'}
      ...
      {schooldatkey != '0' ? <View style={{ marginLeft: 20, marginRight: 20, marginTop: 15, }}>

        <TextField placeholder={'Enter School Address'} onChangeText={val => setaddress(val)} value={address} editable={false}
        />
      </View>
        :
        <View style={{ marginLeft: 20, marginRight: 20, marginTop: 15, }}>

          <TextField placeholder={'Enter School Name'} onChangeText={val => setSchoolname(val)} value={schoolname}
          />
        </View>}
      ...
      {schooldatkey != '0' ? <View style={{ marginLeft: 20, marginRight: 20, marginTop: 15, }}>
        ...
        <TextField placeholder={'Enter Board/University'} onChangeText={val => setBoard(val)} value={Board} editable={false}
        />
      </View>
        :
        <View style={{ marginLeft: 20, marginRight: 20, marginTop: 15, }}>

          <TextField placeholder={'Enter Board/University'} onChangeText={val => setBoard(val)} value={Board}
          />
        </View>}
      ...
      <View style={{ flex:1,marginTop: 30, marginLeft: 20, marginRight: 20 ,marginBottom: 30,}}>
