...
import { CustomButton, CustomStatusBar, Validate, CustomHeader, BackBtn,CustomDropdown,TextField,HeaderTitleWithBack} from '../../../components';
...
const SchoolDetailScreen = (props: AppProps) => {
  ...
  const [Aadhar, setAadhar] = useState('');
    ...
    const CourseAdded = async () => {
        ...
        if (
            coursenameError ||
            dobstartError || dobendError || standardError || standardError1 || desError || schoolidError
        ) {
            //this._scrollView.scrollTo(0);
            Toast.show(coursenameError || dobstartError || dobendError || standardError || standardError1 || desError || schoolidError, Toast.SHORT);
                ...
                userActions.getAddPastEiCourse({
                    ...
                    callback: ({ result, error }) => {
                        ...
                        if (result.status === false) {
                            ...
                            setLoading(false);
                            //console.log('dfdfdf--------', error)
                            Toast.show('Admission Number already available on this School', Toast.SHORT);
    ...
    const CourseAddedOther = async () => {
        ...
        if (
            coursenameError ||
            courseError || dobstartError || dobendError || desError
        ) {
            //this._scrollView.scrollTo(0);
            Toast.show(coursenameError || courseError || dobstartError || dobendError || desError, Toast.SHORT);
    ...
    {coursekey === 0 ? <View>
        ...
        <View style={{ marginTop: '2%', marginBottom: '4%', marginLeft: 2, marginRight: 2 }}>

            <TextField placeholder={'Enter Description'} multiline={true} onChangeText={val => setDess(val)} value={Dess}
            />
        </View>
    </View> : <View>
        ...
        <View style={{ marginTop: '1%', }}>
            <CustomDropdown placeholder={'Select Left Standard'}
                data={selectedJoiningStandard}
        ...
        <View style={{ marginTop: '2%', marginLeft: 2, marginRight: 2, }}>
            ...
            <TextField placeholder={'Enter School ID'} keyboardType='numeric' onChangeText={val => setSchooID(val)} value={S_id}
            />
        </View>

        <View style={{ marginTop: '2%', marginLeft: 2, marginRight: 2, marginBottom: '5%' }}>

            <TextField placeholder={'Add Description of Study...'} multiline={true} onChangeText={val => setDes(val)} editable={false} value={Des}
            />
        </View>
    </View>}
...
<View >
    {coursekey === 0 ? <CustomButton title={'ADD'} onPress={() => CourseAddedOther()} /> : <CustomButton title={'Submit'} onPress={() => CourseAdded()} />}
